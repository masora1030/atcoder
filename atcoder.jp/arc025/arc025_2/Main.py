...
if __name__=='__main__':
    ...
    c1 = [[0] for j in range(w)]
    c2 = [[0 for i in range(w+1)] for j in range(h+1)]
    for i in range(w):
        for j in range(h):
            c1[i].append(c1[i][-1]+c[j][i])
    for i in range(1,h+1):
        for j in range(1, w+1):
            c2[i][j] = c2[i][j-1]+c1[j-1][i]
        ...
        for u in range(1,h+1):
            for r in range(l,w+1):
                for d in range(u,h+1):
                    pre = (r-l+1)*(d-u+1)
                    if c2[d][r]-c2[d][l-1]-c2[u-1][r]+c2[u-1][l-1]==0:
                        ans = max(ans, pre)
