...
if __name__=='__main__':
    ...
    anss = [inf for i in range(n)]
    for k in range(m):
        x, y, d=lr()
        flg = False
        for i,p in enumerate(e[x-1]):
            dd,yy = p
            if yy == y-1 and dd > d:
                e[x-1][i] = [d, y-1]
                flg = True
                break
        for i,p in enumerate(e_inv[y-1]):
            dd,xx = p
            if xx == x-1 and dd > d:
                e_inv[y-1][i] = [d, x-1]
                flg = True
        ...
        if not flg:
            e[x-1].append([d, y-1])
            e_inv[y-1].append([d, x-1])
        if x-1 == y-1:
            anss[x-1] = min(d, anss[x-1])
    ...
    def dijkstra_inv(s):
        hq=[(0, s)]
        heapq.heapify(hq)  # リストを優先度付きキューに変換
        cost=[inf for i in range(len(e_inv))]
        cost[s]=0  # 開始地点は0
        while hq:
            c, v=heapq.heappop(hq)
            if c>cost[v]:  # コストが現在のコストよりも高ければスルー
    ...
    for i in range(n):
        ans = inf
        cost, cost_inv = dijkstra(i), dijkstra_inv(i)
        tmp = inf
        for j in range(n):
            if j != i:
                tmp = min(cost[j]+cost_inv[j], tmp)
        ans=min(ans, anss[i], tmp)
        if ans < inf:
            print(ans)
        else:
            print(-1)
